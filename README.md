...
Previewing never writes to a live SillyTavern message object. In particular, it does not change `mes`, `swipe_id`, `swipes`, `swipe_info`, `extra`, chat metadata, the JSONL source file, or any normal swipe/generation/checkpoint path. No extension settings or chat metadata are persisted.
